...
-- write table name to be able to insert into it
...
CREATE VIEW vw_sec_sub AS
SELECT sec.SectionID,
       sec.SectionName,
       sub.SubjectID,
       sub.SubjectName
FROM Sections sec
JOIN Subjects sub ON sec.SectionID = sub.SectionID; 

-- Insert using SectionSubjectsView: 
--		Add a new subject to a section
INSERT INTO vw_sec_sub(SectionID , 
		SectionName, SubjectID, SubjectName)
VALUES (22, 'Aya',1, 'AI');
-- cannot be done bec cannot determine where to add it
-- specify table name to work
...
-- -->Another similar simple view which will be done similarly
...
GROUP BY c.CourseID, c.CourseName; --both has to be written as above select statement

-- Can we INSERT into this view? 
--		NO, Beacuse we are using a GROUP BY with AGG fn
...
GROUP BY sub.SubjectID, 
			sub.SubjectName,
			p.FirstName,
			p.LastName; --what is not in AGG Fn need to be grouped by
...
-- Can we INSERT into this view? 
--		NO, Beacuse we are using a GROUP BY with Aggregate function
-- Can we UPDATE this view? 
--		NO, Beacuse we are using a GROUP BY with Aggregate function
-- Can we DELETE using this view? 
--		NO, Due to aggregation	
...
CREATE VIEW vw_st_cr_prof AS
SELECT s.StudentID,
       COUNT(DISTINCT e.CourseID) AS total_courses,
	   COUNT(DISTINCT p.ProfessorID ) AS Professors
FROM Students s
JOIN Enrollments e ON s.StudentID = e.StudentID
JOIN CourseSubjects cs ON e.CourseID = cs.CourseID
JOIN Subjects sub ON cs.SubjectID = sub.SubjectID
JOIN Professors p ON sub.ProfessorID = p.ProfessorID
...
-- -->Another similar view which will be done similarly

-- Commands to delete all views
DROP VIEW IF EXISTS vw_sec_sub_enr;
DROP VIEW IF EXISTS vw_sub_gr_avg;
DROP VIEW IF EXISTS vw_st_enr;
DROP VIEW IF EXISTS vw_st_cr_prof;
