...
--Go
--USE foe_db; --change master to foe_db
--Go
--CREATE TABLE Students(
----identity used to control by myself (not the user) the PK where (1,1) is the start and increment
--StudentID INT PRIMARY KEY IDENTITY(1, 1),
----string==NVARCHAR differs from VARCHAR in the encoding
...
--   Email NVARCHAR(100) UNIQUE
--);
--comment after finishing each part to not being executed again
...
--	Capacity INT
--);
--start with the ones that have not any FKs
...
--TRUNCATE TABLE Professors; --cannot be truncated bec another table depends on it
...
--WHERE ProfessorID = 8; constraint (on NULL email only)

--INSERT INTO Sections (SectionName, Capacity)
--VALUES ('Physics Lab', 25);

--DELETE FROM Sections
--WHERE SectionID = 6; constraint "delete what we have created"

--SELECT * FROM Students; *=select all
--constraint:
--WHERE FirstName ='Mostafa'

--SELECT EMAIL FROM Students; select email not all
--constraint:
--WHERE FirstName ='Mostafa'
